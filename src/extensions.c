...
int game_check_deadends(const game_info_t* info,
    ...
    int  dir = 0;
    //getting the last colour of the board and finding the current position
    size_t colour = state->last_color;
    pos_t current = state->pos[colour];
    
   // pos_get_coords(current, &x, &y);

    
    //looping through all the directions
    for(dir = 0; dir < 4; dir++){
        //if the cell is the colour's initial/goal cell then return 0 and mark as "free"
       if(state->cells[current] == TYPE_INIT || state->cells[current] == TYPE_GOAL){
           return 0;
       }
        //if the number of surrounding free cells is equal to one then return 1
        if(game_num_free_pos(info,state, current) == 1){
            return 1;
        }
    }
    //else return 0
